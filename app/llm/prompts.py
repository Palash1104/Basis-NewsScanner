...
SUMMARY_PROMPT_VERSION = "summary-v2"  # v2: single-turn validation retry prompt
...
REGION_LABELS = {"US": "US", "IN": "India", "GLOBAL": "Global"}
...
def render_articles(articles: Sequence[PromptArticle]) -> str:
    ...
    for article in articles:
        ...
        region = REGION_LABELS.get(article.source_region, article.source_region)
        body = escape(article.title, quote=False)
        if article.snippet:
            body += "\n" + escape(article.snippet, quote=False)
        lines.append(
            f'<article source="{escape(article.source_name)}" region="{escape(region)}" '
            f'published="{published}">{body}</article>'
        )
